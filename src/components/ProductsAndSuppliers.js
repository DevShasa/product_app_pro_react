...
export default class ProductsAndSuppliers extends Component{
    ...
    saveData = (collection, item)=>{
        if(item.id === ""){
            ...
            this.setState(state=> state[collection] = state[collection].map(i=> i.id === item.id ? item : i))
        }
    }

    deleteData = (collection, item)=>{
        this.setState(state => state[collection] = state[collection].filter(s => s.id !== item.id))
    }
